...
int main(void)
{
  ...
  if( store_nodes1(&myNode_store) == CONFIG_OK)
  {
  ...
  saved_nodes_t myNode_load ;

  HAL_Delay(1000);

  if(load_nodes(&myNode_load) == CONFIG_OK)
  {
	  if(memcmp(&myNode_store , &myNode_load ,sizeof(saved_nodes_t)) == 0)
	  {
		  printf("TEST VALIDE\n");
	  }
	  else{
	  ...
	  printf("nodes : nb :%d ,addr[0] : %x ,addr[1] : %x\n" ,myNode_load.nb_addresses
			  ,myNode_load.addresses[0],myNode_load.addresses[1]);
  }
  else
  {
	  printf("Invalid data\n");
